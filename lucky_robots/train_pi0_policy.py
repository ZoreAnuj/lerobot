...
class TrainPolicy():
    def __init__(self, repo_id, device=None, n_steps=5000, log_freq=1, max_retries=5, retry_delay=10, pi0_variant="pi0_base"):
        self.repo_id = repo_id
        self.pi0_variant = pi0_variant
        self.output_directory = Path(f"outputs/train/{self.pi0_variant}/{repo_id}_{pi0_variant}")
    ...
    def dataprep(self):
        ...
        # Set policy variant in the configuration
        # This attribute can later be used by DiffusionPolicy if required.
        setattr(self.config, "policy_variant", self.pi0_variant)
        logger.info(f"Using policy variant: {self.pi0_variant}")
    ...
    def train(self):
        ...
        # Create and configure the policy, then move to device
        self.policy = DiffusionPolicy(self.config, dataset_stats=self.dataset_metadata.stats)
        self.policy.train()
        self.policy.to(self.device)
        logger.info("Policy created and moved to device.")
...
def main():
    parser = argparse.ArgumentParser(description="Train a Pi0 policy variant")
    parser.add_argument("--repo_id", type=str, default="abhisb/so100_51_ep", help="Repository ID for the dataset")
    parser.add_argument("--pi0_variant", type=str, choices=["pi0_base", "pi0"], default="pi0_base",
                        help="Select the pi0 policy variant: 'pi0_base' or 'pi0'")
    parser.add_argument("--steps", type=int, default=5000, help="Number of training steps")
    args = parser.parse_args()
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    trainer = TrainPolicy(
        repo_id="Thorns07/so100_test_diffusion_grasp",
        device=device,
        n_steps=args.steps,
        max_retries=10,
        retry_delay=30,
